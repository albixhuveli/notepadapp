import tkinter as tk
from tkinter import ttk, messagebox
import json
from ttkbootstrap import Style

#main window
root = tk.Tk()
root.title("Notes App")
root.geometry("500x500")
style = Style(theme ='journal')
style = ttk.Style()

#config tab font to bold
style.configure("TNoteBook.Tab", font=("TkDefaultFont", 14, "bold"))

#create the notebook to hold notes
notebook = ttk.Notebook(root, style="TNotebook")

#load saved note
notes = {}
try:
    with open("notes.json", "r") as f:
        notes = json.load(f)
except FileNotFoundError:
    pass

#Create notebook to hold notes
notebook = ttk.Notebook(root)
notebook.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)

#func to add new note
def add_note():
    #create tab for new note
    note_frame = ttk.Frame(notebook, padding=10)
    notebook.add(note_frame, text="New Note")

    #create entry widgets for tittle and content of note
    title_label = ttk.Label(note_frame, text="Title:")
    title_label.grad(row=0, column=0, padx=10, pady=10,sticky="W")

    title_entry = ttk.Entry(note_frame, width=40)
    title_entry.grid(row=0, column=1, padx=10, pady=10)

    content_label = ttk.Label(note_frame, text="Content:")
    content_label.grad(row=1, column= 0, padx = 10, pady=10,sticky="W")

    content_entry = ttk.Entry(note_frame, width=40, height=10)
    content_entry.grid(row=0, column=1, padx=10, pady=10)

    #create func to save note
    def save_note():
        # Get the title and content of the ntoe
        title = title_entry.get()
        content = content_entry.get("1.0", tk.END)

        #add note to the notes dictionary
        notes[title] = content.strip()

        #save the ntoes dictionary to the file
        with open("notes.json", "w") as f:
            json.dump(notes, f)
        
        #add note to the notebook
        note_content = tk.Text(notebook, width=40, height=10)
        note_content.insert(tk.END, content)
        notebook.forget(notebook.select())
        notebook.add(note_content, text=title)

    #add a save button to the note frame
    save_button = ttk.button(note_frame, text="Save",
                             command = save_note, style = "secondary.TButton")
    save_button.grid(row=2, column=1, padx=10, pady=10)

def load_notes():
    try:
        with open("notes.json", "r") as f:
            notes = json.load(f)

        for title, content in notes.items():
            #add note to the book
            note_content = tk.Text(notebook, width=40, height=10)
            note_content.insert(tk.END, content)
            notebook.add(note_content, text=title)
    
    except FileNotFoundError:
        #if file doesnt exist
        pass
#call the load_notes func when app starts
load_notes()

#create a func to delete a note
def delete_notes():
    #get the current tab index
    current_tab = notebook.index(notebook.select())

    #get title of the note for deletion
    note_title = notebook.tab(current_tab, "text")

    #show a confirm dialog
    confirm = messagebox.askyesno("Delete Note", 
                                  f"Are you sure you want to delete {note_title}?")
    
    if confirm:
        #remove the note
        notebook.forget(current_tab)

        #remove the note from notes dic
        notes.pop(note_title)

        #save the notes dic to the file
        with open("notes.json", "w") as f:
            json.dump(notes, f)

#add button to the main window
new_button = ttk.Button(root, text="New Note", 
                      command=add_note, style="info.TButton")
new_button.pack(side=tk.LEFT, padx=10, pady=10)

delete_button = ttk.Button(root, text="Delte",
                           command=delete_notes, style="primary.TButton")
delete_button.pack(side=tk.LEFT, padx=10, pady=10)

root.mainloop()




